"""Standalone runner for the Popularity recommender baseline.
...
Usage
-----
    python scripts/popular_rec.py [--data data/processed] [--k 20]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate PopularityRecommender on the Steam dataset.",
...
def main() -> None:
    ...
    model.save(model_path / "popularity_recommender.pkl")
    print(f"  saved to {model_path / 'popularity_recommender.pkl'}")
    ...
    for split_name, users, history, gt in [
        ...
        print(f"\n── PopularityRecommender [{split_name}] ──────────────────────")
